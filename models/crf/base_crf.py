...
from .utils import IMPOSSIBLE_SCORE, log_sum_exp
...
class BaseCRF(nn.Module):
    ...
    def _forward_algorithm(
        ...
        # forward algorithm
            ...
            # NOTE log_proba[0]: (batch_size, num_tags) + (1, num_tags)
        ...
        for i in sequence_iter:
            ...
            # NOTE inner is essentially a batched prob transition table of (batch_size, num_tags, num_tags)
            # from a previous label to the current

            # NOTE log_sum_exp on inner (batch_size, num_tags, num_tags):
            # max_score, _ = inner.max(1, keepdim=False) [from (batch_size, num_tags, num_tags) to (batch_size, 1, num_tags) to (batch_size, num_tags)]
            # stable_vec = inner - max_score.unsqueeze(1) [max_score: from (batch_size, num_tags) to (batch_size, 1, num_tags)]
            # return max_score + (stable_vec.exp().sum(1, False)).log() [(batch_size, num_tags)]
    ...
    def viterbi_decode(self, emissions: torch.Tensor, mask: Optional[torch.ByteTensor] = None) -> torch.FloatTensor:
        """
        Parameters:
            emissions: (batch_size, sequence_length, num_tags)
            mask:  Show padding tags. 0 don't calculate score. (batch_size, sequence_length)
        ...
        # NOTE To generate the same number of labels as sequence_length when mask is present
        # This is to make viterbi_decode compatible with GPT2TokenClassification output where fixed-length logits are expected.
        # Also, the original code may be incorrect, as it seems to imply that masked tokens only appear at the end of the sequence.
        seq_ends = torch.ones([sequence_length, batch_size], dtype=torch.uint8, device=emissions.device).sum(dim=0) - 1
    ...
    def restricted_viterbi_decode(
        self, emissions: torch.Tensor, possible_tags: torch.ByteTensor, mask: Optional[torch.ByteTensor] = None
    ) -> torch.FloatTensor:
        """
        Parameters:
            emissions: (batch_size, sequence_length, num_tags)
